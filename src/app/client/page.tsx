...
export default function ClientPage() {
    ...
    const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        if (isCreated) {
            // edit client
            const res = await axios.post("http://localhost:3000/api/client/edit", {
                clientNo: clientNo,
                firstName: firstName,
                lastName: lastName,
            ...
            // add client
            const res = await axios.post("http://localhost:3000/api/client/add", {
                clientNo: clientNo,
                firstName: firstName,
                lastName: lastName,
                telephone: formData.get('telephone') as string,
    ...
    return (
        <main className="flex flex-col items-center min-h-screen">
            ...
            <Button color="secondary" variant="flat" className="w-32 mb-5" onPress={() => { setSelectedClient(newHireClient); onOpen(); }}>Hire</Button>
                        ...
                        <TableRow key={row.clientNo}>
                            ...
                            <TableCell>
                                <Tooltip content="Edit user">
                                    <Button onPress={() => { setSelectedClient(row); setSelectedPrefType(row.preferredType); onOpen(); }} isIconOnly variant="ghost" className="text-slate-700 cursor-pointer active:opacity-50">
                        ...
                        {selectedClient && (
                            <form className="w-full mx-auto flex flex-col gap-10" onSubmit={onSubmit}>
                                ...
                                <div className="flex flex-col lg:flex-row justify-between gap-4">
                                    ...
                                    <Input required label="Street" labelPlacement="outside" type="tel" name="mobile" defaultValue={selectedClient.street} className="w-full  rounded-lg" />
                                    <Input required label="City" labelPlacement="outside" type="tel" name="mobile" defaultValue={selectedClient.city} className="w-full  rounded-lg" />
